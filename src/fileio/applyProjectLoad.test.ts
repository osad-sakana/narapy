...
describe('applyProjectLoad', () => {
  it('読み込み後に古いエディタ内容が新ファイルへ書き戻されない(issue #45 本命)', () => {
    ...
    expect(openEditorFile).toHaveBeenCalledWith('new.py', 'サーバから読み込んだ新しい内容')
    expect(openEditorFile).not.toHaveBeenCalledWith('new.py', '古いエディタ内容(未保存)')
  })
  ...
  it('呼び出し順序はloadProject → refreshExplorer → エディタ反映(内容/ファイル名/バリデーション)であること', () => {
    ...
    expect(openEditorFile).toHaveBeenCalledWith('new.py', 'print(1)')
    ...
    const editorOrder = openEditorFile.mock.invocationCallOrder[0]
