...
プレーンテキストとしてエスケープ表示されます（`src/exercise/problemPanel.ts`）。
Markdownの書式付きレンダリングは意図的に行っていません（第三者が用意した内容を
`innerHTML` 等で解釈するとXSSにつながるため）。

## 画面表示

演習を開くと、エディタヘッダーに「📘 演習モードで開いています」の表示と
「📄 問題」「✓ 採点」ボタンが現れる（`#exerciseControls`）。問題文はモーダルではなく
エディタ上部の常設パネルに表示され（`#problemPanel`）、「📄 問題」ボタンまたは
パネル右上の×ボタンで開閉をトグルできる。演習の読込直後は自動で開いた状態になる。
通常の`.narapy`プロジェクトへ切り替わると、このパネル・ボタン類は自動的に隠れる。
